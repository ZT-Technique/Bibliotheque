...
@section('page-title', 'Modifier le Slide')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-2xl font-bold text-inoha-black">Modifier le Slide</h1>
            <p class="text-sm text-gray-500 mt-1">Mettez à jour les informations de votre diapositive</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
        <form action="{{ route('admin.sliders.update', $slider) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Image Upload -->
                <div class="md:col-span-2">
                    <label for="image" class="block text-sm font-bold text-gray-700 mb-2">Image du slide <span class="text-gray-400 font-normal">(Laisser vide pour conserver l'image actuelle)</span></label>
                    <div class="relative group">
                        ...
                        <label for="image" class="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer hover:border-inoha-green hover:bg-inoha-green/5 transition-all group-hover:border-inoha-green @error('image') border-rose-500 bg-rose-50 @enderror">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="mb-2 text-sm text-gray-500"><span class="font-bold">Cliquez pour changer d'image</span> ou glissez-déposez</p>
                            </div>
                            <img id="preview" src="{{ route('uploads.serve', ['type' => 'sliders', 'filename' => basename($slider->image_path)]) }}" class="absolute inset-0 w-full h-full object-contain rounded-2xl bg-white p-2 border border-gray-100">
                        </label>
                    </div>
                    @error('image')
                        <p class="mt-2 text-sm text-rose-500 font-medium">{{ $message }}</p>
                ...
                <div class="md:col-span-2">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <div class="relative">
                            <input type="checkbox" name="is_active" class="sr-only peer" {{ $slider->is_active ? 'checked' : '' }} value="1">
                        ...
                        <span class="text-sm font-bold text-gray-700 group-hover:text-inoha-black transition-colors">Slider activé</span>
                    </label>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-4 border-t border-gray-50">
                <button type="submit" id="submit-button" class="flex-1 flex items-center justify-center gap-2 px-8 py-4 bg-inoha-green text-white rounded-xl font-bold hover:bg-inoha-green-dark transition-all shadow-lg shadow-inoha-green/20">
                    ...
                    <span id="button-text">Mettre à jour le slide</span>
                </button>
